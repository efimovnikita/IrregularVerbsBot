...
namespace IrregularVerbsBot
{
    internal static class Program
    {
        private static void Main(string[] args)
        {
            ...
            Option<FileInfo> memoOption = new("--memo", "Memorizer app path")
            {
                IsRequired = true
            };
    ...
    internal class Bot
    {
        ...
        private async Task UpdateHandler(ITelegramBotClient client, Update update, CancellationToken token)
        {
            ...
            if (text == "/start")
            {
                ...
                Queue<string?> queue = new(ShuffleList(output.Split(Environment.NewLine).Where(s => String.IsNullOrWhiteSpace(s) == false).ToArray()));
                ...
                await client.SendTextMessageAsync(chatId, $"Hello {message.Chat.Username}! Let's start!\nGive me a proper past and past participle form of the verbs (separate forms by whitespace).", cancellationToken: token);
                ...
                await client.SendTextMessageAsync(chatId, $"\"{queueElement}\"", cancellationToken: token);
            ...
            {
                await client.SendTextMessageAsync(chatId, "Check function returns error", cancellationToken: token);
                await client.SendTextMessageAsync(chatId, "Done!", cancellationToken: token);
                UserStates.Remove(chatId);
                return;
            }
            ...
            try
            {
                ResultMsg? result = System.Text.Json.JsonSerializer.Deserialize<ResultMsg>(checkOutput);
                if (result == null)
                {
                    await client.SendTextMessageAsync(chatId, $"Something went wrong during checking of the verb \"{verb}\"", cancellationToken: token);
                    return;
                }

                if (result.IsSuccess)
                {
                    await client.SendTextMessageAsync(chatId, "Correct! ", cancellationToken: token);
                }
                else
                {
                    await client.SendTextMessageAsync(chatId, $"Incorrect! The correct answer is: {result.Msg}", cancellationToken: token);
                }
